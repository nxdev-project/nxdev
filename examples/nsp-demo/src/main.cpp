...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== NXDevPack NSP Packaging Demo ===\n\n";
    std::cout << "Application: NSP Packaging Demo v1.0.0\n";
    std::cout << "Title ID:    0100000000000088\n";
    std::cout << "Backend:     hacBrewPack\n\n";
    ...
    if (romfs_res.is_success()) {
        ...
        if (greeting.is_success()) {
            std::cout << "RomFS message: " << greeting.value() << "\n\n";
        } else {
            std::cout << "Notice: greeting.txt not found in RomFS (" << greeting.error_name() << ")\n\n";
        }
    } else {
